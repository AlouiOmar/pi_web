...
class AnnonceMobileController extends Controller
{
    ...
    public function afficherTouteAnnoncesAction(Request $request)
    {
...
//        dump($request->get('active'));
        if($request->get('active') == 'true'){
//            dump('annonce active');
            $annonce->setActive(true);
        }else{
//            dump('annonce Non active');
...
//        die();
        ...
        $annonce->setTypevelo($request->get(null));
        $annonce->setCouleur($request->get(null));
    }
        $annonce->setDate( new \DateTime ($request->get('date')));
//        $date=new \DateTime();
//        $annonce->setDatep($date);
...
//        die();        $annonce->setDescription($request->get('description'));
        $annonce->setPrix($request->get('prix'));
        $annonce->setType($request->get('type'));
//        $annonce->setIdu($user[0]);
        $annonce->setPhoto($request->get('photo'));
