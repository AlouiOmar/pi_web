...
class BlogController extends Controller
{
    ...
    public function updatecommentAction(Request $request, $id,$id_post)
    {
    ...
    public function likeAction(Commentaire $commentaire,$id_post)
    {
    ...
    public function dislikeAction(Commentaire $commentaire,$id_post)
    {
    ...
    public function adminAction()
    {
        ...
        $em = $this->getDoctrine()->getManager();
        $nbruser = $em->getRepository('AppBundle:User');
        $qb = $nbruser->createQueryBuilder('a')
        ->select('count(a.id)')
        ...
        return $this->render('@Blog/Admin/dashbord.html.twig', array(
            'nbrpost' => $nbrpost,'nbrcommentaire'=>$nbrcommentaire,'nbruser'=>$fosuser11));
    }
   ...
   $result = $paginator->paginate(
    ...
    public function modifiercommentadminAction(Request $request, $id)
    {
    $em = $this->getDoctrine()->getManager();
    $transp = $em->getRepository(Commentaire::class)->find($id);
    if ($request->isMethod('POST')) {
    ...
    return $this->render('@Blog/Admin/updatecommentadmin.html.twig', array('transp' => $transp));
    }
...

  {
    ...
    return $this->render('@Blog/Admin/statistique.html.twig', array(
        ...
        {
            $recherche= $request->get('table_search');

            $em = $this->getDoctrine()->getManager();

            $posts = $em->getRepository('BlogBundle:Post')->rechercheActive($recherche);   
        ...
        public function findAdmincommentAction(Request $request)
        {
        $recherche= $request->get('table_search');

        $em = $this->getDoctrine()->getManager();

        $posts = $em->getRepository('BlogBundle:Commentaire')->rechercheActivecomment($recherche);
        ...
        public function findAdminuserAction(Request $request)    
        {
        $recherche= $request->get('table_search');

        $em = $this->getDoctrine()->getManager();

        $fosuser1 = $em->getRepository('AppBundle:User');
        
        $qb = $fosuser1->createQueryBuilder('a')
            ->where('a.firstname like :query or a.age like :query or a.username like :query  or a.telephone like :query  or a.email like :query ')
            ->setParameter('query','%'.$recherche.'%')
            ->getQuery();
      
            $fosuser = $qb->getResult();
